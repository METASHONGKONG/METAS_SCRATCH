...
public class Scratch extends Sprite {
	// Version
	public static const versionString:String = 'v100';
	...
	public var pollObjectManager:PollObjectManager; // Polling Object
	public var serverRequestManager:ServerRequestManager; // Server Request Manager
	public var cloudServer:CloudServer;
	...
	protected function initialize():void {
		...
		initPollObjectManager(); // Need to be before Runtime and ServerRequestManager
		initServerRequestManager(); // Need to be before Interpreter
		initInterpreter();
		initRuntime();
		...
		// if offline, intercept the close window event to check if save needed
		if (isOffline) {
			stage.stage.nativeWindow.addEventListener('closing', closeApplication, false, 0, true);
		}
	...
	protected function initCloudServer():void {
		cloudServer = new CloudServer(this);
	}
	...
	protected function initServerRequestManager():void {
		serverRequestManager = new ServerRequestManager(this);
	}

	protected function initPollObjectManager():void {
		pollObjectManager = new PollObjectManager(this);
	}
	...
	// Log Debug message
	public function logDebug(msg:String, extraData:Object = null):void {
		log(LogLevel.DEBUG, msg, extraData);
	}
...
//		}
	...
	protected function addFileMenuItems(b:*, m:Menu):void {
		...
		// m.addItem('Save Project As', exportProjectToFileAs);
	...
	public function showConnectMenu(b:*):void {
		var m:Menu = new Menu(connectDevice, 'Connect', CSS.topBarColor(), 28);
		var deviceManager:DeviceManager = DeviceManager.instance();
		
		var currentDevices:Array = deviceManager.getDevices();

		if (currentDevices.length == 0) {
			m.addItem(Translator.map('No Serial Port'), 'NIL');
		}
		else {
			for each (var device:String in currentDevices) {
				if (device != deviceManager.getConnectedDevice()) {
					m.addItem(device, device);
				}
				else {
					m.addItem(device, device, true, true); // Add a checkmark
				}
			}
		}
	
		m.addLine();
		m.addItem('Wireless...', function ():void {
			function configWirelessPort(dialog:DialogBox):void {
				var deviceID:String = dialog.getField('Device ID').replace(/^\s+|\s+$/g, '');
				if (deviceID.length == 0) return;

				connectDevice(deviceID);

				// TO-DO: connect the device
				// TO-DO: show the connected IP device
				selectBoard(BoardSelector.METAS_WIRELESS); // Change to WIRELESS by default even connection fails
			}

			var d:DialogBox = new DialogBox(configWirelessPort);
			d.addTitle('Connect to Metas');
			d.addField('Device ID', 120);
			d.addAcceptCancelButtons('Connect');
			d.showOnStage(app.stage);
		});

		// m.addLine();
		m.addItem('IP Address...', function():void {
			function connectViaIPAddress(dialog:DialogBox):void {
				var ipAddress:String = dialog.getField('IP Address').replace(/^\s+|\s+$/g, '');
				if (ipAddress.length == 0) return;

				connectDevice(ipAddress);
			}

			var d:DialogBox = new DialogBox(connectViaIPAddress);
			d.addTitle('Connect via IP Address');
			d.addField('IP Address', 120);
			d.addAcceptCancelButtons('Connect');
			d.showOnStage(app.stage);
		});
	...
	private function connectDevice(item:*):void {
		// call function if it's a function (for wireless)
		if (item is Function) {
			item();	
			return;
		}

		if (item == 'NIL') { // happen when there's no serial port
			return;
		}

		var connectedDevice:String = DeviceManager.instance().getConnectedDevice();
		if (connectedDevice == null) {
			DeviceManager.instance().connect(item);
		}
		else if (item == connectedDevice) {
			...
			/* Don't do that.  It may hang the application if disconnect fails
			DeviceManager.instance().disconnect(function ():void {
				DeviceManager.instance().connect(item);
			});
			*/
		}
	...
	public function setTopBarConnectionStatus(status:Boolean):void {
		topBarPart.setConnectionStatus(status);
	}

	public function showDeviceMenu(b:*):void {
		var m:Menu = new Menu(selectBoard, 'Device', CSS.topBarColor(), 28);
		var board:String = BoardSelector.instance().getSelectedBoard();
	
		var checked:Boolean;
		
		checked = (board == BoardSelector.METAS_3_PORT)? true : false;
		m.addItem(BoardSelector.METAS_3_PORT, BoardSelector.METAS_3_PORT, true, checked);

		checked = (board == BoardSelector.METAS_8_PORT)? true : false;
		m.addItem(BoardSelector.METAS_8_PORT, BoardSelector.METAS_8_PORT, true, checked);

		checked = (board == BoardSelector.METAS_WIRELESS)? true : false;
		m.addItem(BoardSelector.METAS_WIRELESS, BoardSelector.METAS_WIRELESS, true, checked);

		checked = (board == BoardSelector.ARDUINO_UNO)? true : false;
		m.addItem(BoardSelector.ARDUINO_UNO, BoardSelector.ARDUINO_UNO, true, checked);

		m.showOnStage(stage, b.x, topBarPart.bottom() - 1);
	}

	protected function selectBoard(board:String):void {
		BoardSelector.instance().selectBoard(board);	
	...
	public function showAboutDialog(b:*):void {
		DialogBox.notify(
				...
				'\n\nCopyright © 2015 Metas and Coding101' +
	...
	*/
	...
	protected function exitProject():void {
		if (saveNeeded) {
			var dialogBox:DialogBox = new DialogBox();
			dialogBox.addTitle(Translator.map('Project has changed.  Click Exit without Save to exit.'));
			dialogBox.addButton(Translator.map('Exit without Save'), exitApplication);
			dialogBox.addButton(Translator.map('Cancel'), function():void {});
